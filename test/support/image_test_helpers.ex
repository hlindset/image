defmodule Image.TestSupport do
  ...
  # Probe whether the linked libvips was built with JPEG XL (libjxl)
  # save support. On the default bundled libvips `jxlsave_buffer/1` is not
  # even generated by Vix, so guard existence with `function_exported?` and
  # use `apply/3` to avoid an undefined-function compile warning.
  @jxl_supported (function_exported?(Vix.Vips.Operation, :jxlsave_buffer, 1) and
                    match?(
                      {:ok, _},
                      try do
                        apply(Vix.Vips.Operation, :jxlsave_buffer, [@probe_image])
                      rescue
                        _ -> :error
                      end
                    ))
  ...
  @doc """
  Returns true if the linked libvips was built with JPEG XL (libjxl)
  save support. Probed once at compile time.
  """
  def jxl_supported?, do: @jxl_supported
